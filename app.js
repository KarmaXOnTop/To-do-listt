...
let currentPriorityFilter = 'all';  // ← AJOUTE CETTE LIGNE
let allTasks = [];
...
function initFirebase() {
    ...
    try {
        if (firebaseConfig.apiKey === "REMPLACE_MOI") {
            ...
            console.warn("⚠️ Configure Firebase dans app.js !");
            return;
        }
...
function selectUser(user) {
    ...
    // Pré-sélectionner l'assignation
...
function updatePresence(online) {
    ...
    if (online) {
        ...
        // Se déconnecter automatiquement si la page est fermée
...
function listenToTasks() {
    tasksRef.orderByChild('createdAt').on('value', (snapshot) => {
        ...
        // Trier: non complétées en premier, puis par date
        allTasks.sort((a, b) => {
...
function renderTasks() {
    ...
    tasksList.innerHTML = filtered.map(task => `
        <li class="task-item ${task.completed ? 'completed' : ''} by-${task.createdBy?.toLowerCase()}" data-id="${task.id}">
            <div class="task-checkbox" onclick="toggleTask('${task.id}')">
                ${task.completed ? '✓' : ''}
            </div>
            <div class="task-content">
